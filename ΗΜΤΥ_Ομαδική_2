...
def plot_graph(csv_file, output_file, y1_label, y2_label):
    ...
    for row in reader:
        time_range = row[0]
        hour = int(time_range.split(" ")[-1].split(":")[0])  # Extract the hour from the time range
        x.append(hour)
        y1.append(int(row[1]))  # Convert the second column to integer for orange column
        y2.append(-int(row[2]))  # Convert the third column to integer for purple column
...
def main():
    ...
    for csv_file in matching_files:
        ...
        with open(csv_file, 'r') as file:
            header_row = next(csv.reader(file))  # Get the first row of the CSV file
            output_file = f"graph_{header_row[1]}_{header_row[2]}.png"  # Generate output file name
